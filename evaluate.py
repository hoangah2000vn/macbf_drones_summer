...
def build_evaluation_graph(num_agents):
    ...
    s_ref = tf.placeholder(tf.float32, [num_agents, 4])
    ...
    # u is the control action of each agent, with shape [num_agents, 4]
    ...
    def opt_cond(u_res, loop_count, is_safe):
        ...
        # condtion: loop_count is less than refind loop and not safe?
        cond = tf.logical_and(
...
def clip_state(s, x_thres, v_thres=0.1, h_thres=6):
    """ clip the state to within allowed position & velocity bound"""
    x, theta_z = s[:, :3], s[:, 3:] # separate the position (x) and yaw angle (theta_z)
    x = np.concatenate([np.clip(x[:, :2], 0, x_thres),
                        np.clip(x[:, 2:], 0, h_thres)], axis=1)
    s = np.concatenate([x, theta_z], axis=1)
    return s
    ...
    for istep in range(config.EVALUATE_STEPS):
        ...
        for t in range(scene.steps):
            ...
            for i in range(config.INNER_LOOPS_EVAL):
                ...
                s_traj.append(np.expand_dims(s_np[:, [0, 1, 2, 3]], axis=0))
