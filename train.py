...
# in order to use with tensorflow 2.0. tensorflow 2.0 does not have tf.placeholder. must access throug compat
...
def main():
    ...
    with tf.Session() as sess:
        ...
        for istep in range(config.TRAIN_STEPS):
            scene.reset()
            # the initial state
            ### scene_start_points has shape (num_agents, 3) containing the (x, y, z) starting coordinates
            s_np = np.concatenate(
