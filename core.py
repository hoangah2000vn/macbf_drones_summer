...
class Cityscape(object):
    ...
    def reset_starting_points(self):
        """
        Reset starting points of all agents such that an agent is not closed 
        (within a distance < 1) to other agent or to an obstacle.
        """
    ...
    def reset_end_points(self):
        """
        Set end points such that it is not closed (within a distance < 1) to the obstacle
        or the start points
        """
    ...
    def reset_reference_paths(self):
        ...
        if self.show:
            ...
            fig = plt.figure()
        for i in range(self.num_agents):
            sx, sy, sz = self.start_points[i] #start points
            gx, gy, gz = self.end_points[i]   #end points
...
def quadrotor_controller_tf(s, s_ref):
    """
    Take in the current state (s) and the reference state (s_ref) 
    and return the control action
    """
...
def network_cbf(x, r, indices=None):
    ...
    Args:
        x (N, N, 4): The state difference of N agents.
...
def network_action(s, s_ref, obs_radius=1.0, indices=None):
    ...
    Args:
        ...
        s_ref (N, 4): The reference location, velocity and acceleration.
        obs_radius (float): The observation radius.
        indices (N, K): The indices of K nearest agents of each agent.
    Returns:
        u (N, 4): The control action.
    """
    ...
    x = slim.fully_connected(inputs=x,
                                          ...
                                          activation_fn=tf.nn.relu)
    x = slim.fully_connected(inputs=x,
                                          num_outputs=4, # change to 4 outputs
                                          reuse=tf.AUTO_REUSE,
                                          scope='action/fc_4',
                                          activation_fn=None)
...
def remove_distant_agents(x, k, indices=None):
    ...
    #Cityscape.reset_reference_paths(self)